...
test.describe("la démo sans compte", () => {
  ...
  test("④ aucun plafond, aucun compteur pour un visiteur sans compte", async ({ page }) => {
    await bootVisiteur(page);
    const etat = await page.evaluate(() => ({
      compteReel: comptePassioReel(),
      plafondAtteint: plafondPassionsAtteint(),
      restantesPassions: String(passionsRestantesOffertes()),
    ...
    expect(etat.plafondAtteint).toBe(false);
    expect(etat.restantesPassions).toBe("Infinity");
