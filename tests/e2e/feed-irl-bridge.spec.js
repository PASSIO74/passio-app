...
test.describe("Pont Fil → IRL (feed_irl_bridge_v1)", () => {
  ...
  // et MASQUE le CTA historique par CSS — sans jamais le retirer du DOM :
  //     :root.passio-ui-3 .feed-irl-bridge { display: none; }
  // Cette suite-ci a un objet précis : prouver que le pont historique reste
  // INTÉGRALEMENT fonctionnel quand UI-3A est coupée. On pose donc le kill
  // switch documenté `passio_ui_3 = "0"` AVANT le boot (une fois la page
  // chargée, le CTA serait déjà masqué et le clic refusé). La preuve du chemin
  // nominal, UI-3A active, appartient à `ui-v3-passerelle.spec.js` et au test
  // « Rencontrer » de `feed-intents.spec.js`.
