...
//   · l'envoi pose la grande PUIS la légère à `<chemin>.v720.<ext>`, et rend
//     l'URL de la légère ; si la légère échoue, l'URL de la grande, comme avant ;
//   · avatars, couvertures, GIF, vidéos : aucun second objet ;
...
//     et `imageGrande` retrouve l'original.
// ═══════════════════════════════════════════════════════════════════════════
...
test("l'envoi pose la grande puis la légère, et rend l'URL de la légère ; sans légère possible, celle de la grande", async ({ page }) => {
  await preparer(page);
  const r = await page.evaluate(async () => {
    const url = await supaUploadMedia("post_x", "photos", __image(1600, 1200, "jpeg"), "image");
    const envois = __envois.slice(); __envois.length = 0;
    __refuserLegere = true;
    const urlSansLegere = await supaUploadMedia("post_y", "photos", __image(1600, 1200, "jpeg"), "image");
  ...
  expect(r.envois.map(e => e.chemin)).toEqual(["photos/812aee2b-214f-4949-931e-842599b6d68b/post_x.jpg", "photos/812aee2b-214f-4949-931e-842599b6d68b/post_x.v720.webp"]);
  expect(r.envois.map(e => e.type)).toEqual(["image/jpeg", "image/webp"]);
  expect(r.envois[1].octets).toBeLessThan(r.envois[0].octets);
  expect(r.url).toBe("https://passio-app.netlify.app/media/content/photos/812aee2b-214f-4949-931e-842599b6d68b/post_x.v720.webp");
  // Légère refusée : la grande vit seule, sans marqueur — le chemin d'avant.
  expect(r.envoisRefus.map(e => e.chemin)).toEqual(["photos/812aee2b-214f-4949-931e-842599b6d68b/post_y.jpg", "photos/812aee2b-214f-4949-931e-842599b6d68b/post_y.v720.webp"]);
...
test("avatars, couvertures, GIF et vidéos ne reçoivent aucun second objet", async ({ page }) => {
  await preparer(page);
  const r = await page.evaluate(async () => {
    ...
    await supaUploadMedia("post_g", "photos", "data:image/gif;base64,R0lGODlhAQABAAAAACw=", "image");
    return __envois.map(e => e.chemin);
  });
  expect(r).toEqual([
...
test("une légère est servie telle quelle : aucune transformation, la grande se retrouve, les grilles ne demandent plus la grande", async ({ page }) => {
  ...
  const r = await page.evaluate(() => {
    const legere = "https://passio-app.netlify.app/media/content/photos/u/p.v720.webp";
    ...
    return { estLegere: [estImageLegere(legere), estImageLegere(ancienne), estImageLegere(legere + "?x=1"), estImageLegere("photos/u/p.v720.jpg#f")],
      thumbLegere: passioThumb(legere, 700), thumbAncienne: passioThumb(ancienne, 700),
      grande: imageGrande(legere), grandeAncienne: imageGrande(ancienne), src };
  });
  expect(r.estLegere).toEqual([true, false, true, true]);
  expect(r.thumbLegere).toBe("https://passio-app.netlify.app/media/content/photos/u/p.v720.webp");
  expect(r.thumbAncienne).toBe("https://passio-app.netlify.app/media/content/photos/u/p.jpg?width=700&quality=75");
  expect(r.grande).toBe("https://passio-app.netlify.app/media/content/photos/u/p.webp");
  expect(r.grandeAncienne).toBe(r.grandeAncienne);
  expect(r.src).toBe("https://passio-app.netlify.app/media/content/photos/u/p.v720.webp");
