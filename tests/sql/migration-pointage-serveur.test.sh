...
contient "un participant ne peut pas écrire un secret (aucun GRANT INSERT : refus de privilège, avant même la RLS)" "permission denied|row-level security" "$(AUTH_OK "$A" "insert into public.event_checkin_secrets values ('ev_now', 'AAAAAA');")"
