...
function generer() {
  ...
  p("--");
  p("-- ⚠️ POSÉE SI ELLE MANQUE, JAMAIS RECRÉÉE. Le patron habituel du dépôt");
  p("-- (`drop constraint if exists` puis `add`) est ici INAPPLICABLE : quatre");
  p("-- clés étrangères s'appuient sur l'index unique qu'elle crée");
  p("-- (`user_passion_specialties`, `posts`, `stories`, `events`). Au second");
  p("-- passage, le `drop` sort en « cannot drop constraint … because other");
  p("-- objects depend on it » et la migration s'arrête au tiers — mesuré en");
  p("-- l'appliquant deux fois sur un PostgreSQL 16 réel. Un `cascade` serait");
  p("-- pire : il emporterait les quatre clés, qu'on remettrait plus bas… donc");
  p("-- une fenêtre où le contenu n'est plus contraint.");
  p("do $$");
  p("begin");
  p("  if not exists (");
  p("    select 1 from pg_constraint");
  p("     where conname = 'passion_specialties_id_passion_key'");
  p("       and conrelid = 'public.passion_specialties'::regclass");
  p("  ) then");
  ...
  p("--");
  p("-- RETOUR ARRIÈRE COMPLET");
  p("--");
  p("-- ⚠️ CE QU'IL PERD, EXACTEMENT. Il supprime la colonne `specialty_id` :");
  p("-- toute spécialité posée sur une publication PENDANT que le lot tournait");
  p("-- part avec elle, et un nouvel aller n'en retrouvera aucune. Publications,");
  p("-- activités, stories, conversations et profils, eux, sont intacts — leur");
  p("-- `passion_id` n'est pas touché. Vérifié en exécutant ce bloc sur un");
  p("-- PostgreSQL 16 réel : 42 passions redeviennent 19, les 2 publications, la");
  p("-- story et l'activité de test survivent, et plus aucune colonne");
  p("-- `specialty_id` ne subsiste.");
  p("--");
  p("-- Tant que le lot n'a pas tourné en production, `specialty_id` vaut `null`");
  p("-- partout : le retour arrière est alors sans aucune perte.");
  p("--   begin;");
  ["posts", "stories", "events"].forEach(t => {
    p(`--     alter table public.${t} drop constraint if exists ${t}_specialty_fk;`);
    p(`--     alter table public.${t} drop constraint if exists ${t}_specialty_needs_passion;`);
