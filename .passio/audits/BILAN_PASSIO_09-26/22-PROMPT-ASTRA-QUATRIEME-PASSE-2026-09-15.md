...
2. **Séparation des comptes et des échanges** : AUTH-06, MSG-04, MOD-04 (serveur : `abonne_accepte_non_bloque`, migration en prod, preuve comportementale en prod) faits ; MSG-01/SUP-06 inchangés depuis #377 ; **MSG-10 (résidu) : policy de lecture de `conv_messages` « membre ET non bloqué », symétrique — contre-revue, #442 fusionnée, migration EN PRODUCTION et preuve comportementale en production (A bloque B → A lit `m_a,m_c`, B lit `m_b,m_c`, C lit tout, débloquer rend tout ; comptes jetables purgés). MSG-01/SUP-06 (résidu) : l'invitation d'appel devient une ligne `call_invites` (from_id = jeton), la sonnerie part d'un trigger `realtime.send` avec `atteste: true`, un client n'émet plus sur `ring:` — prouvé sur le staging avec un vrai canal Realtime, contre-revue, #447 fusionnée, client servi PUIS migration EN PRODUCTION (ordre imposé : collée avant, les clients en cache ne sonnaient plus), preuve comportementale en production par un vrai canal Realtime : B reçoit `{from: A, atteste: true}`, tiers et usurpation refusés, broadcast client jamais reçu — `preuves/appels/2026-09-15-msg-01-invitation-attestee-production.md`.**
...
8 bis. **IRL, prouvé sur la cible** : IRL-05 — 8 inscriptions concurrentes en PRODUCTION sur 3 places → 3 `going`, 5 `activite_complete`, `waitlist` passe, `going` sur annulée refusé (#449) ; IRL-06 — FK cascade relues en production, l'organisateur supprime → inscription, commentaire, réaction et secret de pointage d'AUTRUI partent (0/0/0/0), un tiers ne supprime rien. Comptes jetables purgés et relus, mêmes exercices sur le staging.
...
PERF-02 (découpage du monolithe, 6 s de JavaScript sur téléphone lent — non corrigé) ; PRO-03 (décision produit) ; TCI-02 (inscription réelle par e-mail sans boîte pilotable) et TCI-07 à 13 ; MSG-06, IRL-10, AUTH-09/10, CONT-11/SUP-01 (décision « 2 ») : consignés avec leur état, non repris ce jour ; rôle modérateur distinct de l'opérateur ; réactions et accusés de lecture d'un membre bloqué non filtrés.

## 5. Ce que je te demande

1. **Contredis les fermetures** — la même méthode que tes deux passes précédentes. Cible en priorité : ASTRA-11/12 (la preuve en production suffit-elle ?), MOD-04 et MSG-10 (symétrie du blocage côté serveur : est-ce le bon choix ?), MSG-01 (une ligne `call_invites` dont `from_id` est le jeton suffit-elle à dire « identité attestée » alors que `call:<id>` reste lisible par tout compte ?), IRL-05/06 (huit requêtes en parallèle depuis un poste prouvent-elles la sérialisation ?), MOD-07 (un jeton accepté depuis le navigateur du propriétaire prouve-t-il l'inscription réelle ?), EXP-01 sur base vide (que manque-t-il encore pour appeler cela une capacité de reprise ?), NET-07 (le journal rétroactif est-il une trace ou une reconstruction ?).
