...
## Reprise du 15/09 — troisième passe d'Astra (fiche 21), état à 14 h 30
...
| ASTRA-20 / point 9 | P2 | le banc vérifie le CONTENU (module pur + 6 tests, contre-épreuve rejouée : 4 × « vide », 0 succès), garde les mesures brutes, et mesure un mélange AUTHENTIFIÉ (lectures, publier, aimer, média, temps réel) sur le staging | ✔ | ✔ 10 413 mesures brutes versionnées ; 10 comptes actifs → p95 ≈ 100 ms, temps réel 420/758 ms ; 25 → queue (publier p99 13 s, 7/90 événements non reçus) ; 50 → p95 ≈ 1,8 s | fusion auto → CI | outil de poste | [#437](https://github.com/PASSIO74/passio-app/pull/437) |
| SUP-10 (résidu) | P1 | **premier run CI staging rouge** : `user_state:user_id=1` — l'état repoussé par le client pendant la purge devenait un « reste » ; client gelé le temps du verdict, reprise serveur d'une écriture tardive | ✔ | ✔ fonction déployée sur le staging, `suppression-compte` 3/3 | fusion auto → CI (la CI déploie `delete-account` en prod) | après déploiement : `supabase functions list` | [#438](https://github.com/PASSIO74/passio-app/pull/438) |
