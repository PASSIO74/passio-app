...
## Reprise du 15/09 — troisième passe d'Astra (fiche 21), état à 13 h
...
| UXO-02 | P1 | **mode « session expirée »** : compte + jeton persisté sans session → fil local en LECTURE, bandeau « Se reconnecter », toute écriture serveur coupée, files gardées ; attente de session au démarrage bornée à 8 s (30 s de blanc hors ligne, mesuré) | ✔ | — | fusion auto → CI | | [#432](https://github.com/PASSIO74/passio-app/pull/432) |
| MOD-07 / SEC-06 | P1 | **widget Turnstile créé** (Cloudflare, `passio-app.netlify.app`, Managed) ; sitekey posée dans app-08, active seulement sur les hôtes déclarés ; l'interrupteur serveur (secret) sera posé APRÈS que le client soit servi | ✔ | — | fusion auto → CI, puis `PATCH config/auth` | | [#433](https://github.com/PASSIO74/passio-app/pull/433) |
| MSG-01/SUP-06, MSG-10, MSG-03, ASTRA-01, CONT-11/SUP-01, MOD-01, AUTH-11/MOD-09, AUTH-09, AUTH-10, MSG-06, IRL-05/06/10, PIL-*, PERF-02/03/05/06, PRO-06, DEV-01/04, TCI-05/06 | PARTIEL / OUVERT | non repris dans cette passe — verdict d'Astra maintenu tel quel (fiche 21) | | | | | |
...
## MOD-07 — Protection contre les faux comptes non prouvée (captcha, limites d'inscription) — P1 (chantier 7) — **widget créé, client en cours de déploiement, interrupteur serveur ensuite**
...
| État | **corrigé dans le code** · déployé : fusion auto → CI · interrupteur serveur : après |
...
| État | **corrigé dans le code** · testé sur staging : non (aucun serveur en jeu) · déployé : fusion auto → CI · vérifié après déploiement : non |
