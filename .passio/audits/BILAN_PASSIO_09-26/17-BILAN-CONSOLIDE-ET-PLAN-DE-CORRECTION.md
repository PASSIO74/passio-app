...
- Couvre : SUP-04 (P0), EXP-11, TCI-04, TCI-01 (suites opt-in), CPL-TCI-A/B (échec silencieux de `mesure-passions.js`, purge Storage ignorée en CI — rapport 16).
...
- Couvre : **SUP-01 / MSG-03 / CONT-11 (P0)**, SUP-02 / MSG-05 (conv_reads), SUP-03 (WITH CHECK conv_messages), AUTH-06 (file locale rejouée sous un autre compte), MSG-10 (bloqué déjà membre), MSG-06 (suppressions qui ressuscitent), SUP-10 partiel (purge Storage à la suppression).
...
- Couvre : AUTH-03, MOD-09, EXP-09 (CGU, mentions, acceptation horodatée), AUTH-04 (télémétrie sans consentement ni interrupteur), EXP-07 (« Support » n'envoie rien), EXP-08 (droits d'accès/portabilité, procédure 72 h), AUTH-05 + SUP-10 (suppression de compte incomplète : ~18 tables + Storage), MOD-07 (captcha, mots de passe compromis).
...
- Couvre : **EXP-01 (P0)**, EXP-02, EXP-04, NET-07, TCI-03, EXP-03.
...
- Couvre : EXP-10 (dépôt public : dossier investisseur, finances, 51 Mo de vidéos), AUTH-01 (gate 2125 cassable en 6 ms, contournable), EXP-05 (une seule personne détient tout).
...
- Couvre : UXO-01 (mur « Installer » iOS à 1,5 s), UXO-02 (landing par-dessus le fil sans session), UXO-03 (messagerie de démonstration non étiquetée), PRO-01 (~900 passions impubliables, `max-rows` 1000 — confirmé aussi côté service_role par le journal CI), CONT-02 (post jamais retenté), CONT-04 (`#user-<id>` non routé), CONT-06 (story « publiée » malgré refus).
