...
| R11 | Domaine d'envoi non authentifié (ni DKIM ni DMARC) → confirmations classées en spam, inscriptions perdues sans trace | Produit/Deliverab. | Élevée | Élevé | **Difficile** (rien ne remonte côté app) | **Posé, non prouvé (2026-09-11 soir)** : les 4 enregistrements DNS résolvent (code Brevo, DKIM `brevo1`/`brevo2`, DMARC `p=none`), Supabase envoie depuis `contact@passio-app.fr` via `smtp-relay.brevo.com:587` (config auth lue par l'API de gestion), un e-mail de récupération a été livré à 13:22 UTC. **Reste la preuve** : une inscription réelle sur un Gmail neuf avec `dkim=pass header.d=passio-app.fr` en boîte de réception — aucune inscription n'a encore traversé le nouveau montage. `docs/SETUP_SMTP_AUTH.md` §3, `docs/OUVERTURE_GRATUITE_GO_NOGO_2026-09-11.md` (EM-1). |
