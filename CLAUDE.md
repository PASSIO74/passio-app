...
- **Notifications cross-compte** : insert via `supaInsertNotif(toUserId, kind, refId, content)` (app-08, RLS INSERT `WITH CHECK (true)` → cross-user OK). Côté réception, `supaLoadNotifications()` était appelé UNIQUEMENT dans le code mort de `supaInit` (jamais chargées !) — corrigé le 2026-06-17 : chargement au boot + à l'ouverture du panneau + canal `realtime:notifications` (postgres_changes filtré `user_id===MY_UID`). Fusion/dédup/badge via `mergeSupaNotifs()`, rendu liste via `_notifListHtml()`. Like/comment/follow notifient l'auteur/la cible.
