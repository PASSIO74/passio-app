...
- Realtime : **v1 (canal global) par défaut**. v2 (canaux **privés par conversation** `conv:<id>`, Broadcast-from-Database) existe (`window.PASSIO_REALTIME_V2`, app-08) mais est **OPT-IN** (`localStorage.passio_realtime_v2 = "1"`) — repassé OFF par défaut le 2026-06-15 car le test 2 comptes a révélé une **course** : pour une conv créée pendant la session, B s'abonne au canal privé (via le handler `realtime:conv_members` → `_subscribePrivateConv`) APRÈS que A ait pu diffuser le 1er message (broadcasts non rejoués) → 1er message perdu. Le `_backfillConvMessages` (handler conv_members) atténue mais ne ferme pas la fenêtre. **Correctif robuste prévu** = topic privé PAR UTILISATEUR (`user:<uid>`, abonné une fois au boot, trigger diffuse à chaque membre) → plus de course. Migration `migration_realtime_authorization.sql` (trigger `conv:<id>` + RLS `realtime.messages`) appliquée en prod. Handler de réception factorisé : `_handleIncomingConvMessage(r)` (commun v1/v2). v1 = canal global `realtime:my_messages` qui reçoit TOUS les messages et filtre l'appartenance en JS (pas de course, mais pas scalable). Garde `window._supaSubscribed` contre le double abonnement (supaInit appelable 2×).
