...
⚠️ **Les cinq autres 401 de cet appareil ne sont pas rouverts ici** : ce sont les signatures du rôle anonyme déjà refermées les 12–13/09 (`_compteAuthReel`, `_uidEstUnCompte`, garde push) ; un appareil qui les produit encore tourne un `app.js` d'avant ces gardes (PWA : l'ancien script vit jusqu'au prochain démarrage complet) ou porte cette divergence `MY_UID`/`window.MY_UID`. Se mesure au `app_version`… qui vaut `2026.08.0` en dur (`PASSIO_APP_VERSION` jamais posé) — point OUVERT : sans version réelle, « ancien client » et « défaut vivant » sont indiscernables au pilotage.
