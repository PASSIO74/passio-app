...
- Realtime : la livraison des INSERT `conv_messages` est confirmée fonctionnelle (testée 2026-06-11) ; le canal reçoit TOUS les messages, le filtrage par membership est applicatif (handler `supaSubscribe`). Garde `window._supaSubscribed` contre le double abonnement (supaInit appelable 2×).
