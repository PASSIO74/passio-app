...
**La restauration.** Aucune n'a été tentée. Il n'existe sur cette machine ni Docker, ni `psql`. ⚠️ **Corrigé le 2026-09-11** : la « base cible » qui manquait **existe depuis le 2026-08-17** — projet Supabase « PASSIO staging » (`fcksxofaelcdmmifnwjo`, même région `eu-west-1`, **en pause**), relevé par l'API de gestion. Ce n'est plus une décision qui manque, c'est l'exercice. Tant que l'opération n'a pas été faite une fois de bout en bout, « on a une sauvegarde » veut dire « on a des fichiers », pas « on sait revenir ».

**Le schéma.** L'archive contient des **données**, pas la structure. La reconstruction suppose de rejouer `migrations/` — or il est établi de longue date sur ce projet que **le schéma de production diverge des migrations du dépôt**. Rejouer les migrations sur une base vide ne redonnerait donc pas nécessairement la production. Cette moitié de la reprise repose aujourd'hui sur les sauvegardes internes de Supabase. ⚠️ **Corrigé le 2026-09-11** : l'organisation est sur le plan **Pro** (pas gratuit, contrairement à ce que plusieurs documents supposent) : **huit sauvegardes physiques quotidiennes** `COMPLETED` (dernière le 2026-09-11 à 04:06 UTC) sont listées par l'API de gestion. Elles existent ; **leur restauration n'a jamais été tentée** (Database → Backups → Restore, projet inaccessible pendant l'opération). Le schéma réel est en cours d'extraction par une autre session (`migrations/00_ORIGINE_PROD.sql`).
