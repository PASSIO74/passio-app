# Bobine Discovery Engine V1 — spécification

> **Statut : NON IMPLÉMENTÉ.** Spécification conservée, fonctionnalité jamais livrée.
>
> Déposée ici le 2026-08-22 lors du regroupement des sessions PR. Elle ne vivait
> jusque-là que dans `.passio/claude-task.md` sur quatre branches-coquilles
> (`claude-task/bobine-discovery-v1`, `-r2`, `-r3`,
> `claude-task/bobine-discovery-v1-release-0821`), dont aucune ne contenait la
> moindre ligne de code produit — et dans aucune version sur `main`.
>
> Quatre tentatives de livraison ont échoué (#67, #78, #85, #100), toutes fermées
> sans fusion. La cause n'était pas la spécification mais le **transport
> d'exécution** : le canal Claude Code n'a jamais persisté de diff sur ces
> branches. Voir le contrat de déclenchement dans `CLAUDE.md`.
>
> **Pourquoi `docs/` et pas `.passio/claude-task.md` :** le workflow
> `.github/workflows/claude-pr-task.yml` lit `.passio/claude-task.md` sur toute
> branche `claude-task/*`. Déposer cette consigne à cet emplacement dans `main`
> la rendrait héritable — et donc ré-exécutable sans que personne l'ait demandé.
> Ici, elle est une référence, pas un ordre.
>
> **État du code au 2026-08-22 :** `buildReels()` (`js/app-05-config-profil.js`)
> reste strictement chronologique. La description « visionneuse chronologique »
> faite ci-dessous est donc toujours exacte. Aucun `passio_reels_discovery_v1`,
> aucun graphe d'adjacence, aucun bucket n'existe dans le dépôt.
>
> ⚠️ Avant toute reprise : `.passio/context/CURRENT_PRIORITIES.md` classe en
> « à ne PAS faire maintenant » tout nouveau gros chantier vertical tant que la
> boucle Feed → IRL n'est pas validée. Cette spécification est conservée pour
> plus tard, elle n'est pas une priorité active.
