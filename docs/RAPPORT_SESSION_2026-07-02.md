...
### 25. Passe ARIA : topbar, toasts, modale (2026-07-05)
Après la bottom-nav (§19) : `#toastStack` en `aria-live="polite"` (toasts annoncés aux lecteurs d'écran), `#modalContent` en `role="dialog" aria-modal="true"`, topbar accessible (logo/Messages/Notifications/Paramètres : `role="button"` + `tabindex="0"` + `aria-label`, SVG en `aria-hidden`), et **activateur clavier générique délégué** (app-08) : tout `[role="button"]` non natif s'active à Entrée/Espace (sans double-activation des nav-item qui ont leur listener). Vérifié en headless : Entrée sur la cloche ouvre les notifications, Espace sur ⋯ ouvre les paramètres sans défiler, 0 erreur console. Reste (continu) : labels des feuilles générées écran par écran.

### 24. TBT ÷ 6 : le bloc app externalisé derrière le gate (perf 86 → ~92)
Le profil CPU de la page verrouillée montrait que le JS applicatif n'exécute presque rien (~100 ms) : le TBT (400 ms) venait du **parse/compile du monolithe inline de 1,1 Mo** que voient tous les nouveaux visiteurs (et Lighthouse) alors que l'app est cachée derrière le gate. `scripts/build.js` émet désormais `dist/app.js` (9 fichiers app + emoji-misc, hash de contenu + `immutable`), injecté par un loader **seulement après `__gateReady`** ; prefetch à la première frappe du code (un preload statique concurrençait le HTML critique). `pwa-landing.js` reste inline. Nouveau spec permanent `dist-build.spec.js` (3 chemins : verrouillé = 0 JS app, code → landing, jeton → boot). Mesuré en local non-minifié : perf 92, TBT 70 ms, FCP 2,5 s. **Résultat PROD déployé (2026-07-05) : Performance 100 · A11y 100 · BP 96 · SEO 100 — FCP 1,4 s · LCP 1,4 s · TBT 30 ms · CLS 0** (avant : 86 / FCP 2,2 s / TBT 400 ms). Rapport archivé : `docs/lighthouse-mobile-2026-07-05.json`. Sessions déjà déverrouillées : injection immédiate, comportement inchangé.
