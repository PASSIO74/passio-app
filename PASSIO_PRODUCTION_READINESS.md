...
| **Storage — écriture** | ✅ **PROUVÉ** | Cloisonné depuis le 2026-08-17 : écrire chez autrui, `move`, `copy`, chemin fabriqué avec `../`, pièce jointe dans une conversation dont on n'est pas membre — tous refusés (≥ 400). Le chemin légitime passe (dépôt, `upsert`, `PUT` sur sa propre clé → 200). Assertions en gate CI |
...
| **Suppression de compte** | ✅ **PROUVÉ** | Le parcours produit est **complet** : vérifié de bout en bout le 2026-08-17 — l'Edge Function supprime le compte (reconnexion refusée ensuite) **et les médias** (servi `200` avant, `400` après). Couvert par `tests/e2e/suppression-compte.spec.js`, mutation-testé. ⚠️ Le résidu observé (76 `user_state`, 54 notifications, 19 médias) vient des suppressions **administratives** qui contournent ce parcours — artefact d'exploitation, pas défaut vécu (`MEDIA-COMPTE-SUPPRIME-010`) |
