...
async function refresh(){const btn=$("refreshBtn");btn.disabled=true;clearError();try{const[command,changes,guardian]=await Promise.all([api("/api/control/command"),api("/api/control/changes"),api("/api/release-guardian")]);renderCommand(command);renderChanges(changes);renderGuardian(guardian);}catch(e){showError(e);}finally{btn.disabled=false;}}
