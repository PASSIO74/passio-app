...
export function computeReadiness({ overview, checklist = [], bugs = [], authz = null }) {
  ...
  const domaines = [
    ...
    {
      cle: "observation",
      label: "Observation (DB + SSE + canari public)",
      critique: true,
      etat: observationEtat(observation.state),
      detail: Object.entries(observation.parts).map(([k, v]) => `${k}:${v.state}`).join(" · "),
      meta: observation,
    },
    {
      cle: "bugs_critiques",
    ...
    {
      cle: "release",
      label: "Release chain (commit → deploy → app → DB)",
      critique: false,
      etat: observationEtat(release.state),
