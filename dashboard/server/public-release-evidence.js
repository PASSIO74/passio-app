...
export function evaluatePublicRelease({ release, expectedCommit = null, expectedBuildId = null, checkedAt = new Date().toISOString(), url = null, error = null }) {
  if (error) return { state: "UNAVAILABLE", checkedAt, url, release: null, error: String(error).slice(0, 300), matches: null };
  if (!release || typeof release !== "object" || !release.buildId) {
    return { state: "UNAVAILABLE", checkedAt, url, release: release || null, error: "release.json invalide ou sans buildId", matches: null };
  }
  const commitMatch = expectedCommit && release.commit ? String(release.commit).startsWith(String(expectedCommit)) || String(expectedCommit).startsWith(String(release.commit)) : null;
  const buildMatch = expectedBuildId ? release.buildId === expectedBuildId : null;
  const mismatches = [];
  if (commitMatch === false) mismatches.push("commit");
  if (buildMatch === false) mismatches.push("buildId");
  return {
    state: mismatches.length ? "MISMATCH" : "LIVE",
    checkedAt, url, release,
...
export async function refreshPublicReleaseEvidence({ force = false, expectedCommit = null, expectedBuildId = null } = {}) {
  ...
  inflight = (async () => {
    ...
    try {
      ...
      cached = evaluatePublicRelease({ error: e?.name === "AbortError" ? "timeout" : (e?.message || String(e)), checkedAt: new Date().toISOString(), url: base + "/release.json" });
    } finally {
...
export function startPublicReleaseEvidence() {
  refreshPublicReleaseEvidence({ force: true }).catch(() => {});
  setInterval(() => refreshPublicReleaseEvidence({ force: true }).catch(() => {}), Math.max(CACHE_MS, 30_000)).unref();
}
