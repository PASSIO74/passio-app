 ...
 */
async function orphans(admin, { child, childKey, parent, parentKey = "id" }) {
  const [c, p] = await Promise.all([loadIds(admin, child, childKey), loadIds(admin, parent, parentKey)]);
  const known = new Set(p.values);
  const missing = c.values.filter((v) => !known.has(v));
  ...
  const real = missing.filter((v) => !isSeedRef(v));
  const seed = missing.length - real.length;
  ...
  return {
    ...
    seedRefs: seed,
    partial: c.truncated || p.truncated,
  };
}
...
const RULES = [
  ...
  },
  {
    ...
    run: (a) => orphans(a, { child: "post_comments", childKey: "post_id", parent: "posts" }),
  },
  {
    ...
    run: (a) => orphans(a, { child: "conv_messages", childKey: "conv_id", parent: "conversations" }),
  },
  {
    ...
    run: (a) => orphans(a, { child: "event_attendees", childKey: "event_id", parent: "events" }),
  },
  {
    ...
    run: (a) => orphans(a, { child: "notifications", childKey: "user_id", parent: "profiles" }),
  },
  {
  ...
  },
  {
    ...
    run: async (a) => {
      const { data, error } = await a.from("posts").select("id").eq("is_reel", true).is("media_url", null).limit(MAX_ROWS);
      if (error) throw new Error("posts : " + error.message);
      const ids = (data || []).map((r) => r.id);
      return { count: ids.length, distinct: ids.length, samples: ids.slice(0, SAMPLE_IDS), partial: ids.length >= MAX_ROWS };
    },
  },
  {
    ...
    run: (a) => orphans(a, { child: "posts", childKey: "author_id", parent: "profiles" }),
  },
];
  ...
  const checks = await Promise.all(RULES.map(async (rule) => {
    try {
      ...
      return {
        ...
        status: r.count > 0 ? rule.severity : "ok",
